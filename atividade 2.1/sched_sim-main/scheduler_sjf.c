...
struct proc *scheduler(struct proc *current)
{
    ...
    if (min_node != NULL) { // Se encontrou um processo com menor tempo restante:
        if (min_prev == NULL) { // Se o processo selecionado é o primeiro da fila:
        ...
        if (min_node->next == NULL) { // e o processo selecionado é o último da fila:
